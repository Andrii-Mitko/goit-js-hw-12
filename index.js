...
</li>`).join("");f.insertAdjacentHTML("beforeend",o),q.refresh()}function $(){f.innerHTML=""}function g(){d.classList.add("visible")}function y(){d.classList.remove("visible")}const u=document.querySelector(".form"),E=document.querySelector(".btn-more");let l=1,n="";u.addEventListener("submit",async r=>{if(r.preventDefault(),n=r.target.elements.search.value.trim(),!!n){$(),g();try{const o=await p(n,l);if(o.hits.length===0){c.warning({message:"Немає результатів для цього запиту",position:"topRight"});return}m(o.hits)}catch{c.error({message:"Помилка запиту",position:"topRight"})}finally{y()}u.reset()}});E.addEventListener("click",async()=>{if(n){l+=1,g();try{const r=await p(n,l);if(r.hits.length===0){c.info({message:"Більше картинок немає",position:"topRight"});return}m(r.hits)}catch{c.error({message:"Помилка запиту",position:"topRight"})}finally{y()}}});
